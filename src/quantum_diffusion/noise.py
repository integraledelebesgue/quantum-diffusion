...
def l1_norm(data: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.normalize(data, p=1, dim=-1)


def l2_norm(data: torch.Tensor) -> torch.Tensor:
    return torch.nn.functional.normalize(data, p=2, dim=-1)


def normalize_mean(target_data: torch.Tensor, inp: torch.Tensor) -> torch.Tensor:
    """
...
def add_uniform_noise_iteratively(
    data: torch.Tensor,
    tau: int,
    decay_mod: float = 1.0,
) -> torch.Tensor:
...
def add_uniform_noise_multiple(
    data: torch.Tensor,
    tau: int,
    decay_mod: float = 2.0,
) -> torch.Tensor:
...
def add_noise_normal_iteratively(
    data: torch.Tensor,
    tau: int,
    decay_mod: float = 0.4,
) -> torch.Tensor:
    ...
    for it in range(1, tau):
        noisy_data[it] = torch.normal(
            mean=noisy_data[it - 1],
            std=noise_weighting[it],
        ).clamp(0, 1)
...
def add_normal_noise_multiple(
    ...
    batch, pixels = data.shape
    noise = torch.normal(mean=0.5, std=0.2, size=(batch, pixels)).to(data.device)
